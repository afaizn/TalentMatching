...
export const authRouter = new Hono()
  ...
  .post("/login", zValidator("json", loginSchema), async (c) => {
    ...
    return c.json({ message: "User login successfully", accessToken, refreshToken });
  })
  .post("/accessToken", async (c) => {
    ...
    try {
      const payload = verifyRefreshToken(refreshToken) as { sub?: number; type?: string };

      // Check if it's a refresh token type
      if (payload.type && payload.type !== "refresh") {
        throw new HTTPException(401, { message: "Invalid token type. Refresh token required." });
      }
